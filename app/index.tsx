...
import { Info, Download, Video, Link2 } from "lucide-react-native";
      ...
      <View className="flex-1 items-center justify-center">
        <View className="w-full max-w-md p-6 space-y-6 rounded-lg bg-card shadow-soft">
          ...
          <View className="flex-row justify-between items-center">
            <Text className="text-xl font-bold text-cardForeground">Video Downloader</Text>
          ...
          <View className="space-y-4">
            <View className="relative w-full">
              ...
              />
              <Pressable className="absolute right-2 p-2"
              style={{ top: 24 }}
              >
            ...
            {!videoData ? (
              <Pressable className="w-full rounded-full py-3 items-center justify-center bg-primary">
                <Text className="text-primaryForeground font-bold">Download</Text>
              </Pressable>
            ) : (
              <View className="space-y-4">
                {/* Thumbnail */}
                ...
                <View className="p-4 bg-secondary/30 rounded-2xl">
                  <View className="flex-row gap-3">
                    ...
                    <View className="flex-1 justify-center">
                      <Text className="text-cardForeground font-medium text-sm">{videoData.title}</Text>
                      <Text className="text-mutedForeground text-xs">{videoData.author}</Text>
                    </View>
                  </View>
                </View>
                ...
                {qualities.map((q, i) => (
                  ...
                  >
                    <View className="flex-row items-center gap-3">
                      ...
                      <Text className="text-cardForeground font-medium">{q.resolution}</Text>
                    </View>
                    <View className="flex-row items-center gap-3">
                      <Text className="text-mutedForeground text-sm">{q.size}</Text>
          ...
          <View className="pt-4 space-y-2 items-center">
            ...
            <Text className="text-xs text-destructive text-center">
              ⚠️ Note: Le téléchargement de vidéos YouTube peut violer leurs conditions d'utilisation.
            </Text>
          </View>
        </View>
      </View>
